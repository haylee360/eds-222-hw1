...
```{r}
# Another way to do the above chunk...
# dataset %>% select(lat, long) %>% 
#   n_distinct
```
...
The population is the air quality (PM 2.5) of the city of Lahore **across the time frame from November 2018 to November 2019**

2.  What are the *samples* in this setting? Please be precise.

The samples are the government and crowdsourced samples comprised of observations from the air quality monitors that were either placed by the government, or in the houses of the volunteers respectively.The government and crowdsourced samples use different methodologies, therefore are two different samples. **The sample occurs over the time period of November 2018 to November 2019.**

The samples are the areas where monitors are installed and collecting data, either for the people or the govt. The fact that the data are collected with different methodologies make these separate samples, so we have a sample of cs data and one for the govt data over time frame.
...
For the government data, there could be bias generated based on where the government chose to put their air quality monitors, intentionally selecting areas with better air quality.

For the crowdsourced data, there could also be volunteer bias based on which households are more or less willing to have an air quality monitor in their homes. There could also be bias introduced by the fact that people in areas with worse air quality are more motivated to volunteer for a monitor.
...
```{r}
# Good practice to keep the set seed there 
set.seed(374)

sample <- comb_df %>% 
  group_by() %>% # doesn't work without empty group_by()
  slice_sample(n=1000)

sample_by_source <- sample %>% 
  group_by(data_source) %>% 
  summarise(count  = n())

kable(sample_by_source,
      caption = "Random sample data sources")
...
**Here, I chose to use `slice_sample()` instead of `sample_n()` The `help` documentation recommends `slice_sample()` and mentions that `sample_n()` has been superseded.**
...
strata_by_source <- strata_comb %>% 
  group_by(data_source) %>% 
  summarise(count  = n())

kable(strata_by_source,
      caption = "Stratified sample data sources")
...
random sample mean = 60.645 stratified sample mean = 66.053 NASA mean = 89.2

The stratified sample matched the satellite data best with a mean of 66.053 $\mu g/m^3$ compared to 89.2 $\mu g/m^3$. I would recommend the prime minister increase funding and development for a stratified sample of air quality primarily using crowd sourced observations. This proposed solution relies more on the crowd sourced data because the crowd sourced mean PM2.5 was even closer to the NASA estimate, with a value of 70.2 $\mu g/m^3$. The crowd sourced data has a larger sample size and there seems to be less influence of bias coming from the crowd sourced data relative to the government data. The government sample mean was shockingly low, clearly demonstrating that there is a large bias coming possibly from government interference/dishonesty/misrepresenting the air quality.
