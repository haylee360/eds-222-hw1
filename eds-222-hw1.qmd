...
The samples are the government and crowdsourced samples comprised of observations from the air quality monitors that were either placed by the government, or in the houses of the volunteers respectively. **The sample occurs over the time period of November 2018 to November 2019. The government and crowdsourced samples use different methodologies, therefore are two different samples**

The samples are the observations from the government air quality monitors and the volunteer household air quality monitors. **The samples are the crowdsourced dataset and the govt dataset. They don't follow the same approach/method.**

The samplea are the areas where monitors are installed and collecting data, either for the people or the govt. The fact that the data are collected with different methodlogies make tehse separate samples, so we have a sample of cs data and one for the govt data over time frame. 
...
For the crowdsourced data, there could also be volunteer bias based on which households are more or less willing to have an air quality monitor in their homes. There could also be bias introduced by the fact that people in areas with worse air quality are more motivated to volunteer for a monitor.  **use bias for both datasets examples**
...
Doesn't show the breakdown of data source for the stratified sampling dataset and used the means from the initial datasets to compare the NASA's mean instead of the subset sampled data.
...
Only looking for answer among the last two... not all of them. 
