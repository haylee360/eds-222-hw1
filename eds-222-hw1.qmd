...
```{r}
combined_data <- bind_rows(crowdsourced, govt, .id = ".id")

comb_df <- comb_df %>% 
  ungroup()

rand_samp_all <- sample_n(comb_df, 1000)
```
...
```{r}
# Round latitudes
sample_round <- combined_data %>% 
  mutate(lat_round = round(latitude, digits = 2))
```

```{r}
# Given number of samples per 0.1 change in latitude
# There are 200 observations per each unique latitude (5)
strata_sample <- sample_round %>% 
  group_by(lat_round) %>% 
  slice_sample(n = 200)

# Check visually
ggplot(strata_sample) +
